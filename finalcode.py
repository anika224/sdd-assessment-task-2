...
def calculate_arithmetic():
    ...
    try:
        result = ((int(arith_num_of_terms))/2)*((2*(int(arith_first_term)))+(int(arith_num_of_terms)-1)*(int(arith_common_diff)))
...
def calculate_geometric(): #geo series calculation
    ...
    try: 
        result = (int(geo_first_term))*((1-(int(geo_common_ratio))**(int(geo_num_of_terms)))/(1-(int(geo_common_ratio))))
...
def clear_entries(): #empties each entry widget
...
def change_theme(theme_name, widgets_to_configure):
    ...
    if theme:
        ...
        geo_first_term_label.config(bg=theme["bg"], fg=theme["fg"])
...
arithmetic_radio.pack(side=tk.LEFT)
geometric_radio.pack(side=tk.LEFT)


#create arithmetic series frame
arithmetic_frame = tk.Frame(root)
arithmetic_frame.pack(pady=30)
...
arith_result_label.pack(pady=10)



#create geometric series frame
geometric_frame = tk.Frame(root)
geometric_frame.pack(pady=30)
...
geo_calculate_button.pack(padx=20, pady=20)

geo_result_label.pack(pady=10)
